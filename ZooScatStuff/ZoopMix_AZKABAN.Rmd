...
pm = data.frame(species=c('Copepods','Euphausiids', 'Chaetognaths'),
  L_dist=c('rnorm', 'rlnorm','rlnorm'),
  L_shape =  c(2.62, 2.4, 2),
  L_rate =   c(0.09, 0.27,  0.4),
  L_a_dist = c('rnorm'),
  L_a_mean = c(2.7, 10.5, 17.15),
  L_a_sd =   c(0.2,  0.3, 3),
  g_dist = 'rnorm',

  g_mean = c(0.996,1.037, 1.028),
  g_sd = c(0.003,0.005, 0.005),
  h_dist = 'rnorm',
  h_mean = c(1.027,1.026, 1.028),
  ...
  theta_sd =   c(30, 20,   30),
  rho_l =    c(  3,   3,    3),
  taper =    c(  2,   4,    2) #taper =2 L/a>2 prolate spheroid, others selected from ZooScat Ex.
)
```
...
setdf = do.call('rbind',list(cdf,kpdf,chdf))
...
```{r export-feather}
path <- "AZKABAN_ZoopMix_data.feather"
