---
title: "Running a DWBA simulation ensemble for Cod Larvae in Tromsøflaket"
author: "Muriel Dunn"
date: "`r Sys.Date()`"
output:
...
fname <- paste0("/Users/mbd/phd/ZooScatStuff/config_CodLarvae.dat") #Location of the parameters file
...
```{r output}
# Change into a table 
df_TS=data.frame(res_TS$y)
df_freq=data.frame(res_TS$var)

path <- "my_codlarvae_TS.feather"
write_feather(df_TS, path)

path <- "my_codlarvae_freq.feather"
write_feather(df_freq, path)
```


## SIMULATIONS!
```{r run_sim_auto}
Species_name = "CodLarvae"
...
res <- bscat(para=para, misc=misc, simOut = TRUE, nang=15, nl=150) #Target strength vs Frequency
```
